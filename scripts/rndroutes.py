...
def generate_flows(route, num=1, min_time=0, max_time=1, mu=0.5, sigma=0.5):
    ...
    for i in xrange(num):
        ...
        period = (10 + max(0, random.normalvariate(0.5, 1.0)) * 100) * num
...
class Edge:
    ...
    def add_link(self, to_edge):
        self.linked_edges.append(to_edge)

    def get_linked_edges(self):
        return list(self.linked_edges)
...
def generate_routes(net, num=1, max_hop=10):
    ...
    for i in xrange(num):
        ...
        # edge つなげてく
        while True:
            linked_edges = edge.get_linked_edges()
            remaining_edges = [linked_edge for linked_edge in linked_edges if not route.has_edge(linked_edge)]
            if len(remaining_edges) <= 0 or max_hop < hop:
                break
